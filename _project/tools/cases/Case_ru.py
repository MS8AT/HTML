...
<header>
  <div class="top-bar">
    ...
    <div class="top-buttons">
      <!-- EN version -->
      <a href="../../en/cases/cases.html" class="dealer-btn">ENG</a>

      <!-- Home RU -->
      <a href="../index.html" class="dealer-btn">Главная</a>

      <!-- Dealers RU -->
      <a href="../dealer/dealer.html" class="dealer-btn">Дилеры</a>
    </div>
  </div>

  <div class="nav-buttons">
    <a href="./cases.html">Корпуса</a>
    <a href="../cooler/cooler.html">Кулеры</a>
    <a href="../cpu/cpu.html">Процессоры</a>
    <a href="../mb/mb.html">Материнские платы</a>
    <a href="../minipc/minipc.html">Mini-PC</a>
    <a href="../monitor/monitor.html">Мониторы</a>
    <a href="../printer/printer.html">Принтеры</a>
    <a href="../soft/soft.html">Софт</a>
    <a href="../svga/svga.html">Видеокарты</a>
...
<main>
  <div class="search-box">
    <input type="text" id="searchInput" placeholder="Поиск модели или форм-фактора...">
  </div>
...
"""
    ...
    # Генерация карточек корпусов для русской версии
    for case in cases_data:
        model_no = case['ModelNo']
        category = case['category']
        image_url = get_image_url(model_no, case.get('main_photo'))
        
        # Формирование описания на русском языке
        description_parts = []
        if case.get('type'):
            description_parts.append(f"Тип: {case['type']}")
        if case.get('color'):
            description_parts.append(f"Цвет: {case['color']}")
        if case.get('material'):
            description_parts.append(f"Материал: {case['material']}")
        
        description = ", ".join(description_parts)
        
        html += f"""
    <div class="case-card" data-category="{category}">
      <img src="{image_url}" alt="Корпус {model_no}" onerror="this.src='../images/cases/placeholder.png'">
      <h3>Корпус {model_no}</h3>
      <p>{description}</p>
    </div>
"""
...
<footer>
  ...
  <script>
    // Функция фильтрации карточек без использования JSON
    ...
    function openModal(item) {
        ...
        // Собираем спецификации из описания
        const description = item.querySelector('p').textContent;
        const specs = description.split(', ').map(spec => spec.trim());
